...
int main()
{
    ...
    {
        auto msg = text.c_str();
        send(connection_socket, msg, MESSAGE_LENGTH, 0); // така€ херн€, надо точно переписать
        std::getline(std::cin, text);
    }

    closesocket(connection_socket); // в случае удачного завершени€ операции возвращает нулевое значение.
    WSACleanup();
}
...
int load_library() {
    ...
    // ѕровер€ем, что загрузили нужную версию
